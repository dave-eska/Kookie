...
void Player::Move(float dt){
    ...
    int mod = direction == DIRECTION_LEFT ? body.width/8 : 0;
...
std::string Player::getDirection(){
    return EnchantToStringMap[direction];
}
...
Player::Player(const char* texture_path, Vector2 pos, int speed,
    ...
    float halfWidth = ((body.width/2.5) / 2.0f) / PIXELS_PER_METER;
    float halfHeight = ((body.height/5) / 2.0f) / PIXELS_PER_METER;
